...
- **URL** — direct link QR
- **vCard** — contact card
- **WiFi** — scans nearby networks, select yours, enter password, generates a scannable connection code
...
## Network Tab — Six Diagnostics Tools (Requires Internet)
...
- **DNS Lookup** — resolves A, AAAA, MX, TXT, CNAME, NS records
- **My IP** — shows public IPv4 and IPv6, all active local interface IPs, ISP info and approximate location
...
Output formats: JPEG, PNG, WebP, AVIF, GIF, BMP, TIFF, ICO, TGA.
...
Open the Network tab, select My IP. Public IPv4/IPv6, all local interface IPs, and ISP info appear on one screen.
