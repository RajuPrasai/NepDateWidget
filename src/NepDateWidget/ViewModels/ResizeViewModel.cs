...
public sealed class ResizeViewModel : ViewModelBase
{
    ...
    public bool ShowStretchWarning
    {
        get
        {
            if (!uint.TryParse(_widthText, out uint w) || w == 0)
            {
                return false;
            }

            if (!uint.TryParse(_heightText, out uint h) || h == 0)
            {
                return false;
            }
    ...
    public bool CanResize
    {
        get
        {
            if (Files.Count == 0 || _isJobRunning || _isJobComplete || HasMixedTypeWarning || IsPdfLoaded)
            {
                return false;
            }
